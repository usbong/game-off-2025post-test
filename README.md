...
I've gotten the hero to stop his movement when he's hit a wall tile. He can also move away from the wall, though this part would still need further testing, because, while somewhat rare, he does get stuck to the wall and would have to moved to the opposite direction several times using the left-mouse click.
...
I've been able to create another html file ("start.html") that serves as the container for the main html ("index.html"), thereby allowing me to quickly center horizontally and/or vertically the app. I also admit that seeing itch.io do this gave me the idea that it's already doable.
...
I attempted to auto-set the scale factor based on the height of the device that the user is using; however, I learned that it was simpler to set it to 75% on Android, while keep it as is on the iPad due to my being unable to determine the height of both the address and the tab bars of the browser, thereby causing the scale factor to be off by several pixels.

I've also solved the problem of the hero snapping his head when he attempts to walk away after punching a monster and then having to auto-face the monster again to punch him before reaching his walking destination. I did this by using `bIsActionKeyControlPressed`, which is the `boolean` variable that I check whether the hero is strafing or not, to force the hero to continuously face the same direction after hitting the monster, until after walking for half his body `width` or `height`. He also stops auto-strafing when he is able to successfully slay the monster.
...
I came across another bug despite already making the hero move diagonally. This time, it was when the hero bumped upward instead of horizontally, left or right. I removed the part of the code that forcibly made the hero move diagonally, and learned that the key concern that I had since the beginning was that the hero isn't immediately pushed backward upon hitting a side of the wall. This didn't appear to be a problem that I had when I was using directional keypads, except during the time when I was developing for the Nokia series 40, a feature phone model whose processor and memory could do better than a two color-indexed snake game pre-installed in the earlier models, but seemed to fail to handle very well multipe key presses in rapid succession. In this case, I attributed the problem to my using the mouse.

As it turns out, I found that I could call the `updateArrayActorsViewport()` function to immediately update the change or difference in the hero's position upon movement including after getting pushed back by the wall, instead of waiting for the hero to incrementally move toward the desired `x` an `y` positions, something that didn't work during my prior attempts. 
...
The new punching animation frames I created when the hero is facing diagonally turned out to be reusable when the hero is facing directly left or right. Using `iCurrHeroComboCount`, as indicated by the value displayed to the right of `Cx` on the screen, I varied the punching animations after the hero has made a straight punch two times. At the moment, however, the diagonal punching animations do not yet change. 
...
I've finally gotten the hero's attacking animation to move through each frame of animation as I had originally intended, something that I had to cut short during Game Off 2025, because, as I was reminded of today, looking for the cause of the problem used up quite a bit of my time, in fact, about half my afternoon, which amounted to a good two hours with only some slight interruptions in-between due to incoming phone calls, among other things. 

I pinpointed the problem to an `on-off switch` involving whether or not `bHasFired` has been set to `false`. Every time I did a left-click with the mouse, `bHasFired` became `true`, which stopped the attacking animation counter from incrementing, thereby causing the hero's animation frame to stay as is. 

During Game Off 2025, I think that I simply opted to use only one attacking frame of animation, which returned to the default "idle" frame immediately after, and then added the particle and the explosion effects to somewhat hid the lack of animation frames when the hero attacked.
